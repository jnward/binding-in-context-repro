...
# %%
def patch_all_acts_at_positions(
    target_ids: torch.Tensor,
    target_cache: ActivationCache,
    vectors: dict[int, torch.Tensor],
):
    def position_patch_hook(
        activation: torch.Tensor, hook, hook_name: str, vectors: dict[int, torch.Tensor]
    ):
    ...
    corrupt_logits = model.run_with_hooks(
        ...
        fwd_hooks=[
            (
                hook_name,
                partial(position_patch_hook, hook_name=hook_name, vectors=vectors),
            )
            for hook_name in hooks_of_interest
        ],
    )
...
for example in tqdm(capitals_examples[:n_examples]):
    ...
    answer_token_ids = torch.stack(
        [
            model.tokenizer.encode(
                f" {answer}", return_tensors="pt", add_special_tokens=False
            ).squeeze()
            for answer in [answer_0, answer_1]
        ]
    ).to(device)

    tokenized_0 = model.tokenizer.encode(context_plus_E0_query, return_tensors="pt").to(
        device
    )
    tokenized_1 = model.tokenizer.encode(context_plus_E1_query, return_tensors="pt").to(
        device
    )
    ...
    with torch.no_grad():
        ...
        corrupt_A_logits_0 = patch_all_acts_at_positions(
            tokenized_0, target_cache, {A_0_POS: mean_A_diff, A_1_POS: -mean_A_diff}
        )[0, -1, answer_token_ids]
        tokenized_0[:, : CONTEXT_LENGTH - 1] = 3
        z2 = patch_all_acts_at_positions(
            tokenized_0, target_cache, {A_0_POS: mean_A_diff, A_1_POS: -mean_A_diff}
        )[0, -1, answer_token_ids]
        assert torch.allclose(corrupt_A_logits_0, z2)
        # print(z2)
        corrupt_A_logits_1 = patch_all_acts_at_positions(
            tokenized_1, target_cache, {A_0_POS: mean_A_diff, A_1_POS: -mean_A_diff}
        )[0, -1, answer_token_ids]
        corrupt_E_logits_0 = patch_all_acts_at_positions(
            tokenized_0,
            target_cache,
            {
                E_0_POS: mean_E_diff,
                E_0_POS + 1: mean_E_next_diff,
                E_1_POS: -mean_E_diff,
                E_1_POS + 1: -mean_E_next_diff,
            },
        )[0, -1, answer_token_ids]
        corrupt_E_logits_1 = patch_all_acts_at_positions(
            tokenized_1,
            target_cache,
            {
                E_0_POS: mean_E_diff,
        ...
        corrupt_both_logits_0 = patch_all_acts_at_positions(
            tokenized_0,
            target_cache,
            {
                A_0_POS: mean_A_diff,
        ...
        corrupt_both_logits_1 = patch_all_acts_at_positions(
            tokenized_1,
            target_cache,
            {
                A_0_POS: mean_A_diff,
